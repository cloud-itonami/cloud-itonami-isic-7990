(ns reservationops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (ReservationOpsAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the ReservationGovernor (:govern)
  ...
  human operator/reservation-desk manager. The approver resumes with
...
(defn build
  ...
  `reservationops.store/Store`).
  opts:
    :advisor      -- a `reservationops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ReservationOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; ReservationGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :reservation-id (:reservation-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :reservation-id (:reservation-id request) :by (:by approval)}]}
            {:disposition :hold
